...
<section class="gs-panel access"><div class="gs-title"><div><h2>Доступ к настройкам</h2><p>Исполнитель определяется по персональному коду и не вводится вручную.</p></div><span :class="unlocked?'open':'locked'">{{unlocked?(session.name||"РАЗБЛОКИРОВАНО"):"ТОЛЬКО ЧТЕНИЕ"}}</span></div>
...
<section class="gs-panel"><div class="gs-title"><div><h2>Объект и пороговые зоны</h2><p>Черновик формы не перезаписывается фоновым обновлением состояния.</p></div><span v-if="dirty" class="draft">НЕ СОХРАНЕНО</span></div>
...
<div class="settings"><div v-for="gas in gases" :key="gas.key" class="gas"><h3>{{gas.name}}</h3><label>Красная → жёлтая<input type="number" step="0.1" v-model.number="gas.warnLow" :disabled="!unlocked" @input="markDirty"></label><label>Жёлтая → зелёная<input type="number" step="0.1" v-model.number="gas.okLow" :disabled="!unlocked" @input="markDirty"></label><label>Зелёная → жёлтая<input type="number" step="0.1" v-model.number="gas.okHigh" :disabled="!unlocked" @input="markDirty"></label><label>Жёлтая → красная<input type="number" step="0.1" v-model.number="gas.warnHigh" :disabled="!unlocked" @input="markDirty"></label></div></div>
<div class="global"><label>Максимум шкалы<input type="number" step="0.1" v-model.number="displayMax" :disabled="!unlocked" @input="markDirty"></label><label>Гистерезис<input type="number" step="0.1" v-model.number="hysteresis" :disabled="!unlocked" @input="markDirty"></label><label>Исполнитель<input :value="session.name||'—'" disabled></label><button @click="save" :disabled="!unlocked||!dirty">Проверить и сохранить</button></div><p class="message" :class="{error:!success}">{{message}}</p></section>
...
const flow = [
  ...
  {id:"catch-runtime",type:"catch",z:tab,name:"Runtime errors",scope:[ids.pollSequencer,ids.influxWriteHttp,ids.historyHttp,ids.eventsHttp,ids.maxHttp,ids.authUsersHttp],uncaught:false,x:190,y:520,wires:[["fn-error-log"]]},
