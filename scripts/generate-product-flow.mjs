...
<section class="current-panel"><div class="gs-title"><div><h2>Токовые входы WB-MAI6</h2><p>Фактический ток петли, обновление в реальном времени</p></div></div><div class="current-grid"><article v-for="input in ['IN1P','IN2P','IN3P','IN4P','IN5P']" :key="input" :class="{'is-live':health.currents&&health.currents[input]&&Number.isFinite(health.currents[input].currentMa)&&Date.now()-Number(health.currents[input].updatedAt)<Math.max(10000,pollIntervalMs*2),'is-low':health.currents&&health.currents[input]&&Number.isFinite(health.currents[input].currentMa)&&Date.now()-Number(health.currents[input].updatedAt)<Math.max(10000,pollIntervalMs*2)&&health.currents[input].currentMa<3.5}"><strong>{{input}}</strong><span>{{health.currents&&health.currents[input]&&Number.isFinite(health.currents[input].currentMa)&&Date.now()-Number(health.currents[input].updatedAt)<Math.max(10000,pollIntervalMs*2)?health.currents[input].currentMa.toFixed(3)+' mA':'—'}}</span><small>{{!health.currents||!health.currents[input]||!Number.isFinite(health.currents[input].currentMa)||Date.now()-Number(health.currents[input].updatedAt)>=Math.max(10000,pollIntervalMs*2)?'не опрашивается / нет данных':health.currents[input].currentMa<3.5?'обрыв / нет сигнала':health.currents[input].currentMa<4?'валидный ноль':'сигнал в диапазоне'}}</small></article></div></section>
...
<section class="config-card"><h3>Общие параметры</h3><div class="fields three"><label>Максимум шкалы, бар<input type="number" step="0.1" v-model.number="displayMax" :disabled="!unlocked" @input="markDirty"></label><label>Гистерезис, бар<input type="number" step="0.1" v-model.number="hysteresis" :disabled="!unlocked" @input="markDirty"></label><label>Период полного цикла опроса, мс<input type="number" min="1000" max="10000" step="500" v-model.number="pollIntervalMs" :disabled="!unlocked" @input="markDirty"><small class="field-hint">Рекомендуется 1000–3000 мс при 115200 bit/s. Ниже 1000 мс недостаточно проверенного запаса; выше 3000 мс медленнее обнаруживаются аварии. Значения вне 1000–10000 мс не сохраняются.</small></label></div></section>
