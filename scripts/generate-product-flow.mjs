...
<style>
.nrdb-ui-group.gm-group .nrdb-layout-group--grid{height:calc(100dvh - 48px)!important;grid-template-rows:minmax(0,1fr)!important}
...
.gm-shell{height:calc(100dvh - 48px);min-height:560px;box-sizing:border-box;display:grid;grid-template-rows:auto minmax(0,1fr) auto;overflow:hidden;padding:24px;color:#eef6ff;background:radial-gradient(circle at 15% 0,#173b59 0,transparent 34%),#071521;font-family:Inter,Segoe UI,sans-serif}
...
<main class="gh-shell"><header><div><p>RINIR · архив измерений</p><h1>История давления</h1></div><nav><a href="/dashboard/monitoring">Мониторинг</a><a href="/dashboard/events">События</a></nav></header>
...
for (const gas of gases) {
  ...
  events.push({payload:{kind:"gas-state-change",key:gas.key,name:gas.name,value:gas.value,from:from || "startup",to:gas.status,reason:gas.reason,durationMs,updatedAt:changedAt}});
}
...
return {payload:{kind:"history",gas,points,error:"",limits:msg.historyLimits,windowMs:msg.historyWindowMs}};`;
...
const prefix=event.kind==="gas-reminder"?"НАПОМИНАНИЕ":"RINIR";
msg.payload={text:prefix+" · "+event.name+"\\n"+labels[event.from]+" → "+labels[event.to]+"\\n"+value,notify:true};
...
const tags=",gas="+escTag(gas)+",name="+escTag(name)+",from="+escTag(from)+",to="+escTag(to)+",reason="+escTag(event.reason||"none")+",operator="+escTag(event.operator||"system")+",has_value="+hasValue;
