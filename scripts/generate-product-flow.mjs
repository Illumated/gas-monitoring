...
const identity={monitorId:String(env.get("MONITOR_ID")||"").trim(),siteName:String(env.get("SITE_NAME")||"").trim(),locationName:String(env.get("LOCATION_NAME")||"").trim()};
...
const siteName=env.get("SITE_NAME");
const locationName=env.get("LOCATION_NAME");
msg.method = "POST";
...
msg.historyIdentity={monitorId:String(env.get("MONITOR_ID")||"").trim(),siteName:String(env.get("SITE_NAME")||"").trim(),locationName:String(env.get("LOCATION_NAME")||"").trim()};
...
const tags=",monitor_id="+escTag(env.get("MONITOR_ID"))+",site="+escTag(env.get("SITE_NAME"))+",location="+escTag(env.get("LOCATION_NAME"))+",gas="+escTag(gas)+",name="+escTag(name)+",from="+escTag(from)+",to="+escTag(to)+",reason="+escTag(event.reason||"none")+",operator="+escTag(event.operator||"system")+",has_value="+hasValue;
...
const identity={monitorId:String(env.get("MONITOR_ID")||"").trim(),siteName:String(env.get("SITE_NAME")||"").trim(),locationName:String(env.get("LOCATION_NAME")||"").trim()};const response=(extra={})=>({payload:{kind:"engineering",identity,settings:flow.get("runtimeSettings")||current,health,unlocked:Number(unlocks[client])>Date.now(),...extra}});
