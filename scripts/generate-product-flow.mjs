...
<section class="current-panel"><div class="gs-title"><div><h2>Токовые входы WB-MAI6</h2><p>Фактический ток петли, обновление в реальном времени</p></div></div><div class="current-grid"><article v-for="input in ['IN1P','IN2P','IN3P','IN4P','IN5P']" :key="input" :class="{'is-live':Number.isFinite(health.currents?.[input]?.currentMa)&&Date.now()-Number(health.currents?.[input]?.updatedAt)<Math.max(10000,pollIntervalMs*2),'is-low':Number.isFinite(health.currents?.[input]?.currentMa)&&Date.now()-Number(health.currents?.[input]?.updatedAt)<Math.max(10000,pollIntervalMs*2)&&health.currents[input].currentMa<3.5}"><strong>{{input}}</strong><span>{{Number.isFinite(health.currents?.[input]?.currentMa)&&Date.now()-Number(health.currents?.[input]?.updatedAt)<Math.max(10000,pollIntervalMs*2)?health.currents[input].currentMa.toFixed(3)+' mA':'—'}}</span><small>{{!Number.isFinite(health.currents?.[input]?.currentMa)||Date.now()-Number(health.currents?.[input]?.updatedAt)>=Math.max(10000,pollIntervalMs*2)?'не опрашивается / нет данных':health.currents[input].currentMa<3.5?'обрыв / нет сигнала':health.currents[input].currentMa<4?'валидный ноль':'сигнал в диапазоне'}}</small></article></div></section>
...
<section class="config-card"><h3>Общие параметры</h3><div class="fields three"><label>Максимум шкалы, бар<input type="number" step="0.1" v-model.number="displayMax" :disabled="!unlocked" @input="markDirty"></label><label>Гистерезис, бар<input type="number" step="0.1" v-model.number="hysteresis" :disabled="!unlocked" @input="markDirty"></label><label>Период полного цикла опроса, мс<input type="number" min="2000" max="10000" step="500" v-model.number="pollIntervalMs" :disabled="!unlocked" @input="markDirty"><small class="field-hint">Рекомендуется 2000–5000 мс. Ниже 2000 мс возможна очередь Modbus; выше 5000 мс медленнее обнаруживаются аварии. Значения вне 2000–10000 мс не сохраняются.</small></label></div></section>
<section class="config-card"><h3>Modbus-оборудование</h3><div class="fields two"><label>Unit ID WB-MAI6<input type="number" min="1" max="247" step="1" v-model.number="valves.wbMai6UnitId" :disabled="!unlocked" @input="markDirty"></label><label>Unit ID WB-MR3LV/I<input type="number" min="1" max="247" step="1" v-model.number="valves.unitId" :disabled="!unlocked||!valves.enabled" @input="markDirty"></label></div><p class="hint">За одним шлюзом — не более одного модуля каждого типа.</p></section>
</div>
...
const pollBuilderCode = `const settings=flow.get("runtimeSettings")||{};const configuredInterval=Number(settings.pollIntervalMs);const fallbackInterval=Number(env.get("MODBUS_POLL_INTERVAL_MS"))||2000;const pollIntervalMs=Number.isInteger(configuredInterval)&&configuredInterval>=2000&&configuredInterval<=10000?configuredInterval:fallbackInterval;const now=Date.now();const lastPollAt=Number(context.get("lastPollAt"))||0;if(now-lastPollAt<pollIntervalMs)return null;context.set("lastPollAt",now);const configured=Number(settings.valves?.wbMai6UnitId);const fallback=Number(env.get("MODBUS_UNIT_ID"))||65;const unitid=Number.isInteger(configured)&&configured>=1&&configured<=247?configured:fallback;const enabled=new Set(Array.isArray(settings.gases)&&settings.gases.some(gas=>typeof gas.enabled==="boolean")?settings.gases.filter(gas=>gas.enabled).map(gas=>gas.key):["oxygen","air","vacuum"]);const requests=[
...
health.currents = health.currents || {};
health.currents[input] = {currentMa:Number.isFinite(currentMa)?Math.round(currentMa*1000000)/1000000:null,updatedAt:Date.now()};
...
if (!Number.isFinite(currentMa) || nanoamps === 2147483647 || nanoamps === -2147483648 || currentMa < 3.5) {
  ...
  const payload={key:msg.topic,code:gas.code,name:gas.name,value:null,raw:null,status:"nodata",reason:Number.isFinite(currentMa)&&currentMa<3.5?"loop-current-low":"invalid",limits,updatedAt:Date.now()};
  return [{payload},null,{payload:{input,currentMa:Number.isFinite(currentMa)?currentMa:null,pressureBar:null,status:"nodata",reason:payload.reason,updatedAt:payload.updatedAt}}];
}
...
return [{payload},{payload},{payload:{input,currentMa,pressureBar:value,status,reason:null,updatedAt:payload.updatedAt}}];`;
...
const pollMs = Number.isInteger(configuredPollMs) && configuredPollMs >= 2000 && configuredPollMs <= 10000 ? configuredPollMs : Math.max(2000, Number(env.get("MODBUS_POLL_INTERVAL_MS")) || 2000);
...
const flow = [
  ...
  {id:ids.pollDelay,type:"delay",z:tab,name:"Serialize Modbus requests",pauseType:"rate",timeout:"5",timeoutUnits:"seconds",rate:"1",nbRateUnits:"0.3",rateUnits:"second",randomFirst:"1",randomLast:"5",randomUnits:"seconds",drop:false,allowrate:false,outputs:1,x:650,y:180,wires:[[ids.pollGetter]]},
  ...
  {id:ids.normalize,type:"function",z:tab,name:"Validate, scale and classify",func:normalizeCode,outputs:3,timeout:0,noerr:0,initialize:"",finalize:"",libs:[],x:470,y:180,wires:[[ids.state],[ids.influxWrite],["debug-wb-mai6-current"]]},
  ...
  {id:ids.engineeringManager,type:"function",z:tab,name:"Engineering access and settings",func:engineeringManagerCode,outputs:3,timeout:0,noerr:0,initialize:"",finalize:"",libs:[],x:520,y:860,wires:[[ids.engineeringUi],[ids.eventWrite],[ids.authUsersHttp]]},
