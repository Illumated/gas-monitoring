...
<section class="config-card"><h3>Общие параметры</h3><div class="fields two"><label>Максимум шкалы, бар<input type="number" step="0.1" v-model.number="displayMax" :disabled="!unlocked" @input="markDirty"></label><label>Гистерезис, бар<input type="number" step="0.1" v-model.number="hysteresis" :disabled="!unlocked" @input="markDirty"></label></div></section>
...
const pollBuilderCode = `const settings=flow.get("runtimeSettings")||{};const configured=Number(settings.valves?.wbMai6UnitId);const fallback=Number(env.get("MODBUS_UNIT_ID"))||65;const unitid=Number.isInteger(configured)&&configured>=1&&configured<=247?configured:fallback;const enabled=new Set(Array.isArray(settings.gases)&&settings.gases.some(gas=>typeof gas.enabled==="boolean")?settings.gases.filter(gas=>gas.enabled).map(gas=>gas.key):["oxygen","air","vacuum"]);const requests=[
  ["oxygen",5380],["air",9476],["vacuum",13572],["n2o",17668],["co2",21764],["valvefeedback",25860]
].filter(([name])=>name==="valvefeedback"||enabled.has(name));return [requests.map(([name,address])=>({topic:name,modbusRequest:{name},payload:{fc:4,unitid,address,quantity:1}}))];`;
...
const flow = [
  ...
  {id:ids.pollCycle,type:"inject",z:tab,name:"Gas polling cycle",props:[{p:"payload"}],repeat:"2",crontab:"",once:true,onceDelay:1,topic:"",payload:"",payloadType:"date",x:150,y:180,wires:[[ids.pollBuilder]]},
