...
|---|---|---|
| `normal` | 50 / 52 / 48 / 50 / 50 | все каналы `ok`, обратная связь `1` |
| `zero` | 0 на пяти входах | 0.0 bar, `alarm`, запись в историю |
| `warning` | 38 / 62 / 39 | 3.8 / 6.2 / 3.9 bar, `warn`, включая переход из `alarm` с гистерезисом |
| `oxygenalarm` | O₂=20, остальные в норме | только O₂ `alarm`, обратная связь `0` |
| `alarm` | аварийные значения пяти входов | активные каналы `alarm` |
| `valvealarm` | газы в норме, IN6P=0 | отдельный аварийный режим клапанов |
| `nodata` | 32767 | `nodata`, значение не пишется в InfluxDB |

Сценарии `calibration4`, `calibration12` и `calibration20` выставляют для пяти газовых входов raw `0`, `50` и `100`. Они нужны только для проверки процедуры `hardware-fat.mjs`; физическую точность тракта они не подтверждают.
...
5. Убедиться, что свежие точки всех активных газов снова появились в InfluxDB.
...
- runtime-flow содержит цикл `1000 ms`, задержку очереди `100 ms` и опрос только отмеченных в UI каналов; фактический интервал для наборов из 3, 4 и 5 каналов проверяется FAT;
