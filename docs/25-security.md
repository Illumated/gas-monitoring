...
- `nftables` разрешает входящий `443/tcp` только через `rinir-mgmt`; на `rinir-modbus` нет удалённого dashboard.
...
- Debian Installer не создаёт обычного пользователя и не оставляет доступного root-пароля: техническое значение `*` блокируется повторно в `late_command`; локальный `rinir-kiosk` создаётся provisioning-скриптом без административного интерактивного доступа.
