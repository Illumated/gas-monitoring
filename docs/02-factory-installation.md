...
Для заводского режима «подключить флешку и включить питание» необходимо заранее установить USB первым в UEFI Boot Order. В конце установки создаётся `/var/lib/rinir-factory/install.done`. При следующей загрузке с флешки UEFI-меню на 30 секунд показывает два пункта: безопасный `RINIR uzhe ustanovlen - zagruzit sistemu`, выбранный по умолчанию, и явное предупреждение `VNIMANIE: polnostyu steret disk i pereustanovit RINIR`. Ранняя загрузочная среда использует ASCII-транслитерацию, поскольку кириллица поддерживается не всеми UEFI GRUB. Только второй пункт передаёт installer параметр `rinir_force_reinstall=true`; без него независимая проверка перед разметкой запрещает очистку диска с маркером `install.done`.
