...
После пересборки monitoring повторно проверен в Chrome при viewport 1366×768 и 1920×1080. В обоих случаях размер документа и рабочей области равен viewport, внешней рамки и прокрутки нет; короткий заголовок и отдельная плашка времени отображаются корректно. На FAT-профиле визуально проверены `НОРМА`, `ВНИМАНИЕ` и `АВАРИЯ`: крупные значения принимают цвет состояния, увеличенные плашки не перекрывают названия каналов. Engineering ранее проверен при 1366×768; строки идентификации и поля сервисной формы отображаются корректно.
