...
| Проверка | Результат |
|---|---|
| Flow audit | 40 объектов, SHA-256 `334BE5E18C3B54D9F375CD4FD98D8172B41C2B749E7C1E286D16A39A22233D17` |
...
После пересборки monitoring повторно проверен в Chrome при viewport 1366×768 и 1920×1080. В обоих случаях размер документа и рабочей области равен viewport, внешней рамки и прокрутки нет; короткий заголовок и отдельная плашка времени отображаются корректно. На FAT-профиле визуально проверены `НОРМА`, `ВНИМАНИЕ` и `АВАРИЯ`: крупные значения принимают цвет состояния, увеличенные плашки не перекрывают названия каналов. Аварийный контур меняется за цикл 2,4 с от слабого до умеренного свечения без мигания содержимого; при `prefers-reduced-motion` используется статичный контур. Engineering ранее проверен при 1366×768; строки идентификации и поля сервисной формы отображаются корректно.
